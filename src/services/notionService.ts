...
export class NotionService {
  ...
  async initialize(credentials: NotionCredentials): Promise<boolean> {
    try {
  ...
  async getDatabases(): Promise<NotionDatabaseInfo[]> {
    if (!this.client) {
  ...
  async getDatabasePages(
    ...
    pageSize: number = 50,
    startCursor?: string,
  ): Promise<{ pages: NotionPageData[]; hasMore: boolean; nextCursor?: string }> {
    if (!this.client) {
      throw new Error('Notion client not initialized');
    }
    ...
    try {
      ...
      const response = await (this.client as any).databases.query(query);

      const pages: NotionPageData[] = [];

      for (const page of response.results) {
        if (page.object === 'page') {
          const pageData = await this.processPage(page as any);
          pages.push(pageData);
        }
      }
  ...
  private async processPage(page: any): Promise<NotionPageData> {
  ...
  private extractPageTitle(page: any): string {
    ...
    for (const [, value] of Object.entries(properties)) {
      if ((value as any).type === 'title') {
        return this.extractRichText((value as any).title);
      }
    }
  ...
  private processProperties(properties: Record<string, any>): Record<string, any> {
    ...
    for (const [key, prop] of Object.entries(properties)) {
      ...
      switch (propData.type) {
        ...
        case 'select':
          ...
          break;
        case 'multi_select':
          processed[key] = propData.multi_select?.map((item: any) => item.name) || [];
          break;
        case 'date':
        ...
        case 'status':
          ...
          break;
        case 'relation':
          processed[key] = propData.relation?.map((item: any) => item.id) || [];
          break;
        case 'people':
          processed[key] = propData.people?.map((person: any) => person.name) || [];
          break;
        case 'created_time':
  ...
  static convertToPersonalData(page: NotionPageData, dataType: string): any {
    const baseData = {
      _id: `notion_${page.id}`,
      dataType,
      source: 'notion',
